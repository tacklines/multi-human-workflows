...
async fn test_create_plan() {
    ...
    let slug = format!("plan-{}", Uuid::new_v4().to_string()[..8].to_string());
    sqlx::query(
...
async fn test_plan_status_default() {
    ...
    let plan_id = Uuid::new_v4();
    let slug = format!("default-{}", Uuid::new_v4().to_string()[..8].to_string());
    ...
    .bind(plan_id).bind(project_id).bind(user_id)
    .bind("Default Status Plan").bind(&slug)
    .execute(&db).await.unwrap();

    let row: (String, String) = sqlx::query_as(
        "SELECT status, body FROM plans WHERE id = $1"
    )
    .bind(plan_id)
    .fetch_one(&db).await.unwrap();
...
async fn test_plan_slug_uniqueness() {
    ...
    .bind(Uuid::new_v4()).bind(project_id).bind(user_id)
    .bind("Second Plan").bind(&slug)
    .execute(&db).await;

    assert!(result.is_err(), "Duplicate slug within same project should be rejected by unique constraint");
}
...
async fn test_plan_update_status() {
    ...
    let plan_id = Uuid::new_v4();
    let slug = format!("status-{}", Uuid::new_v4().to_string()[..8].to_string());
    sqlx::query(
        "INSERT INTO plans (id, project_id, author_id, title, slug, created_at, updated_at)
         VALUES ($1, $2, $3, $4, $5, NOW(), NOW())"
    )
    .bind(plan_id).bind(project_id).bind(user_id)
    .bind("Status Transition Plan").bind(&slug)
    .execute(&db).await.unwrap();
...
async fn test_plan_parent_child() {
    ...
    let parent_slug = format!("parent-{}", Uuid::new_v4().to_string()[..8].to_string());
    sqlx::query(
    ...
    .bind("Original Architecture").bind(&parent_slug)
    .execute(&db).await.unwrap();

    // Create child plan that supersedes the parent
    let child_id = Uuid::new_v4();
    let child_slug = format!("child-{}", Uuid::new_v4().to_string()[..8].to_string());
    sqlx::query(
    ...
    let children: Vec<(Uuid, String)> = sqlx::query_as(
        "SELECT id, title FROM plans WHERE parent_id = $1"
    )
    .bind(parent_id)
...
async fn test_plan_project_scoping() {
    ...
    sqlx::query("INSERT INTO organizations (id, name, slug, created_at) VALUES ($1, $2, $3, NOW())")
        .bind(org_id_b).bind("Org B").bind(format!("org-b-{}", Uuid::new_v4()))
        .execute(&db).await.unwrap();

    let project_id_b = Uuid::new_v4();
    sqlx::query("INSERT INTO projects (id, org_id, name, slug, created_at) VALUES ($1, $2, $3, $4, NOW())")
        .bind(project_id_b).bind(org_id_b).bind("Project B").bind(format!("proj-b-{}", Uuid::new_v4()))
        .execute(&db).await.unwrap();

    let shared_slug = format!("shared-{}", Uuid::new_v4().to_string()[..8].to_string());
    ...
    .bind(Uuid::new_v4()).bind(project_id_a).bind(user_id)
    .bind("Plan in A").bind(&shared_slug)
    .execute(&db).await.unwrap();
    ...
    .bind(Uuid::new_v4()).bind(project_id_b).bind(user_id)
    .bind("Plan in B").bind(&shared_slug)
    .execute(&db).await.unwrap();

    // Each project should see only its own plan
    let plans_a: Vec<(String,)> = sqlx::query_as(
        "SELECT title FROM plans WHERE project_id = $1 AND slug = $2"
    )
    .bind(project_id_a).bind(&shared_slug)
    .fetch_all(&db).await.unwrap();
    assert_eq!(plans_a.len(), 1);
    assert_eq!(plans_a[0].0, "Plan in A");

    let plans_b: Vec<(String,)> = sqlx::query_as(
        "SELECT title FROM plans WHERE project_id = $1 AND slug = $2"
    )
    .bind(project_id_b).bind(&shared_slug)
    .fetch_all(&db).await.unwrap();
