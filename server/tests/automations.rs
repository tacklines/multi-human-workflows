...
async fn test_create_event_reaction() {
    ...
    let (name, event_type, enabled): (String, String, bool) =
        sqlx::query_as("SELECT name, event_type, enabled FROM event_reactions WHERE id = $1")
            .bind(reaction_id)
            .fetch_one(&db)
            .await
            .unwrap();
...
async fn test_event_reaction_project_scoping() {
    ...
    let count_a: i64 =
        sqlx::query_scalar("SELECT COUNT(*) FROM event_reactions WHERE project_id = $1")
            .bind(project_a)
            .fetch_one(&db)
            .await
            .unwrap();
...
async fn test_delete_event_reaction() {
    ...
    let exists: bool =
        sqlx::query_scalar("SELECT EXISTS(SELECT 1 FROM event_reactions WHERE id = $1)")
            .bind(reaction_id)
            .fetch_one(&db)
            .await
            .unwrap();
...
async fn test_create_scheduled_job() {
    ...
    let (name, cron, enabled): (String, String, bool) =
        sqlx::query_as("SELECT name, cron_expr, enabled FROM scheduled_jobs WHERE id = $1")
            .bind(job_id)
            .fetch_one(&db)
            .await
            .unwrap();
...
async fn test_scheduled_job_project_scoping() {
    ...
    let count_a: i64 =
        sqlx::query_scalar("SELECT COUNT(*) FROM scheduled_jobs WHERE project_id = $1")
            .bind(project_a)
            .fetch_one(&db)
            .await
            .unwrap();
...
async fn test_scheduled_job_last_run_tracking() {
    ...
    let last_run: Option<chrono::DateTime<chrono::Utc>> =
        sqlx::query_scalar("SELECT last_run_at FROM scheduled_jobs WHERE id = $1")
            .bind(job_id)
            .fetch_one(&db)
            .await
            .unwrap();
...
async fn test_event_bridge_cursor_singleton() {
    ...
    // Update cursor
    sqlx::query(
        "UPDATE event_bridge_cursor SET last_event_id = 42, updated_at = NOW() WHERE id = 1",
    )
    .execute(&db)
    .await
    .unwrap();
    ...
    let last_id: i64 =
        sqlx::query_scalar("SELECT last_event_id FROM event_bridge_cursor WHERE id = 1")
            .fetch_one(&db)
            .await
            .unwrap();
    ...
    let result = sqlx::query("INSERT INTO event_bridge_cursor (id, last_event_id) VALUES (2, 0)")
        .execute(&db)
        .await;
