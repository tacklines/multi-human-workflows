...
async fn test_org_dek_one_per_org() {
    ...
    sqlx::query("INSERT INTO org_credential_keys (org_id, encrypted_dek) VALUES ($1, $2)")
        .bind(org_id)
        .bind(b"fake-dek-bytes".as_slice())
        .execute(&db)
        .await
        .unwrap();
...
async fn test_org_dek_cascade_on_org_delete() {
    ...
    let exists: bool =
        sqlx::query_scalar("SELECT EXISTS(SELECT 1 FROM org_credential_keys WHERE org_id = $1)")
            .bind(org_id)
            .fetch_one(&db)
            .await
            .unwrap();
...
async fn test_org_credential_invalid_type() {
    ...
    assert!(
        result.is_err(),
        "Invalid credential_type should be rejected by CHECK constraint"
    );
}
...
async fn test_org_credential_unique_name_per_org() {
    ...
    assert!(
        result.is_err(),
        "Duplicate credential name per org should be rejected"
    );
}
...
async fn test_org_credential_same_name_different_orgs() {
    ...
    assert_eq!(
        count, 2,
        "Different orgs should be able to have credentials with the same name"
    );
}
...
async fn test_org_credential_cascade_on_org_delete() {
    ...
    let count: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM org_credentials WHERE org_id = $1")
        .bind(org_id)
        .fetch_one(&db)
        .await
        .unwrap();
...
async fn test_user_credential_unique_name_per_user() {
    ...
    assert!(
        result.is_err(),
        "Duplicate credential name per user should be rejected"
    );
}
...
async fn test_user_dek_cascade_on_user_delete() {
    ...
    let dek_exists: bool =
        sqlx::query_scalar("SELECT EXISTS(SELECT 1 FROM user_credential_keys WHERE user_id = $1)")
            .bind(user_id)
            .fetch_one(&db)
            .await
            .unwrap();
    ...
    assert_eq!(
        cred_count, 0,
        "User credentials should cascade-delete with user"
    );
}
...
async fn test_org_credential_custom_env_var() {
    ...
    let env_var: Option<String> =
        sqlx::query_scalar("SELECT env_var_name FROM org_credentials WHERE id = $1")
            .bind(cred_id)
            .fetch_one(&db)
            .await
            .unwrap();
