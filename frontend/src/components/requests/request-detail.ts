...
const TRANSITIONS: Record<RequestStatusType, Transition[]> = {
  pending: [
    { label: 'requestDetail.transition.analyze', status: 'analyzing', variant: 'warning' },
    { label: 'requestDetail.transition.archive', status: 'archived', variant: 'neutral' },
  ],
  analyzing: [
    { label: 'requestDetail.transition.decompose', status: 'decomposed', variant: 'success' },
    { label: 'requestDetail.transition.archive', status: 'archived', variant: 'neutral' },
  ],
  decomposed: [
    { label: 'requestDetail.transition.archive', status: 'archived', variant: 'neutral' },
  ...
  archived: [
    { label: 'requestDetail.transition.reopen', status: 'pending', variant: 'primary' },
  ],
};
...
export class RequestDetail extends LitElement {
  ...
  updated(changed: Map<string, unknown>) {
    if ((changed.has('requestId') || changed.has('projectId')) && this.requestId && this.projectId) {
      this._load();
    }
  }
  ...
  private async _load() {
    ...
    try {
      ...
      this._error = err instanceof Error ? err.message : t('requestDetail.errorLoad');
    } finally {
      this._loading = false;
    }
  }
  ...
  private async _saveEdit() {
    ...
    try {
      ...
      if (this._editTitle !== this._request.title) updates.title = this._editTitle;
      if (this._editBody !== this._request.body) updates.body = this._editBody;
      if (Object.keys(updates).length > 0) {
        this._request = await updateRequest(this.projectId, this.requestId, updates);
      }
      this._editing = false;
    } catch (err) {
      this._error = err instanceof Error ? err.message : t('requestDetail.errorSave');
    } finally {
      this._saving = false;
    }
  }
  ...
  private async _transition(status: string) {
    ...
    this._error = '';
    try {
      this._request = await updateRequest(this.projectId, this.requestId, { status });
    } catch (err) {
      this._error = err instanceof Error ? err.message : t('requestDetail.errorStatus');
    } finally {
  ...
  render() {
    ...
    if (!this._request) {
      return html`<div style="text-align: center; color: var(--text-tertiary); padding: 2rem;">${t('requestDetail.notFound')}</div>`;
    }
    ...
    return html`
      ...
      <div class="header">
        <sl-button class="back-btn" size="small" variant="text"
                   @click=${() => this.dispatchEvent(new CustomEvent('request-back', { bubbles: true, composed: true }))}>
      ...
      <div class="section">
        ...
        ${this._editing ? html`
          <sl-textarea rows="6" value=${this._editBody}
                       @sl-input=${(e: CustomEvent) => { this._editBody = (e.target as HTMLTextAreaElement).value; }}
          ></sl-textarea>
          <div class="edit-controls">
            <sl-button size="small" variant="default" @click=${() => this._cancelEdit()}>${t('requestDetail.cancel')}</sl-button>
      ...
      <div class="section" style="margin-top: 1.5rem;">
        <div class="section-label">${t('requestDetail.linkedRequirements')} (${r.linked_requirement_ids.length})</div>
        ${r.linked_requirement_ids.length > 0 ? html`
          <div class="chips">
            ${r.linked_requirement_ids.map(id => html`<span class="chip">${id.slice(0, 8)}</span>`)}
          </div>
        ` : html`
