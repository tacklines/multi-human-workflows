...
const TRANSITIONS: Record<RequirementStatusType, Transition[]> = {
  draft: [
    { label: 'requirementDetail.transition.activate', status: 'active', variant: 'primary' },
    { label: 'requirementDetail.transition.archive', status: 'archived', variant: 'neutral' },
  ],
  active: [
    { label: 'requirementDetail.transition.satisfy', status: 'satisfied', variant: 'success' },
    { label: 'requirementDetail.transition.archive', status: 'archived', variant: 'neutral' },
  ],
  satisfied: [
    { label: 'requirementDetail.transition.reopen', status: 'active', variant: 'primary' },
    { label: 'requirementDetail.transition.archive', status: 'archived', variant: 'neutral' },
  ],
  archived: [
    { label: 'requirementDetail.transition.reopen', status: 'draft', variant: 'primary' },
  ],
};
...
export class RequirementDetail extends LitElement {
  static styles = css`
    ...
    .child-row:not(:last-child), .task-row:not(:last-child) {
  ...
  updated(changed: Map<string, unknown>) {
    if ((changed.has('requirementId') || changed.has('projectId')) && this.requirementId && this.projectId) {
      this._load();
    }
  }
  ...
  private async _load() {
    ...
    try {
      ...
      this._error = err instanceof Error ? err.message : t('requirementDetail.errorLoad');
    } finally {
      this._loading = false;
    }
  }
  ...
  private async _saveEdit() {
    ...
    try {
      ...
      if (this._editDesc !== this._req.description) updates.description = this._editDesc;
      if (Object.keys(updates).length > 0) {
        this._req = await updateRequirement(this.projectId, this.requirementId, updates);
      }
      this._editing = false;
    } catch (err) {
      this._error = err instanceof Error ? err.message : t('requirementDetail.errorSave');
    } finally {
      this._saving = false;
    }
  }
  ...
  private async _transition(status: string) {
    ...
    this._error = '';
    try {
      this._req = await updateRequirement(this.projectId, this.requirementId, { status });
    } catch (err) {
      this._error = err instanceof Error ? err.message : t('requirementDetail.errorStatus');
    } finally {
  ...
  render() {
    ...
    if (!this._req) {
      return html`<div style="text-align: center; color: var(--text-tertiary); padding: 2rem;">${t('requirementDetail.notFound')}</div>`;
    }
    ...
    return html`
      ...
      <div class="header">
        <sl-button class="back-btn" size="small" variant="text"
                   @click=${() => this.dispatchEvent(new CustomEvent('requirement-back', { bubbles: true, composed: true }))}>
      ...
      <div class="body-section">
        ${this._editing ? html`
          <sl-textarea rows="8" value=${this._editDesc}
                       @sl-input=${(e: CustomEvent) => { this._editDesc = (e.target as HTMLTextAreaElement).value; }}
                       placeholder=${t('requirementDetail.descPlaceholder')}
          ></sl-textarea>
          <div class="edit-controls">
            <sl-button size="small" variant="default" @click=${() => this._cancelEdit()}>${t('requirementDetail.cancel')}</sl-button>
        ...
        ` : html`
          ${r.description ? html`<p style="margin: 0; color: var(--text-primary); font-size: 0.9rem; white-space: pre-wrap;">${r.description}</p>` : html`
            <div class="empty-body">${t('requirementDetail.emptyDesc')}</div>
          `}
        `}
      </div>
        ...
        ${r.children.length > 0 ? html`
          <div class="child-list">
            ${r.children.map(c => html`
              <div class="child-row">
                <sl-icon name="bullseye" style="color: var(--text-tertiary); font-size: 0.8rem;"></sl-icon>
                <span class="child-title">${c.title}</span>
                <sl-badge variant=${STATUS_VARIANTS[c.status]}>${t(STATUS_LABEL_KEYS[c.status])}</sl-badge>
                <sl-badge variant=${PRIORITY_VARIANTS[c.priority]}>${t(PRIORITY_LABEL_KEYS[c.priority])}</sl-badge>
              </div>
            `)}
          </div>
        ` : html`<div class="empty-sub">${t('requirementDetail.noChildren')}</div>`}
      </div>
      ...
      <div class="sub-section">
        ...
        ${r.linked_task_ids.length > 0 ? html`
          <div class="task-list">
            ${r.linked_task_ids.map(id => html`
              <div class="task-row">
                <sl-icon name="check2-square" style="color: var(--text-tertiary); font-size: 0.8rem;"></sl-icon>
                <span class="task-id">${id.slice(0, 8)}</span>
              </div>
            `)}
          </div>
        ` : html`<div class="empty-sub">${t('requirementDetail.noTasks')}</div>`}
      </div>
    `;
  }
}
